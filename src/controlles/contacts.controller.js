...
const onRemoveFriend = async (req, res, next) => {
    try {
        ...
        await CONTACT_SERVICE.onRemoveContact({ ownerId, friendId })
...
const onGetContactsOnlines = async (req, res, next) => {
    try {
        const userId = req.user.id
        const data = await CONTACT_SERVICE.onGetContactOfUserOnline(userId)
        res.status(200).json({
            data: data
        })
    } catch (error) {
